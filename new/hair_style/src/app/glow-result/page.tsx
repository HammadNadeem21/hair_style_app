    ...
    if (!glowResult) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#F8FAFC]">
                <Sparkles className="text-primaryColor mb-4 animate-pulse" size={48} />
                <h2 className="text-xl font-bold text-gray-800">No analysis found</h2>
                <p className="text-gray-500 text-center mt-2">Please upload a photo and start the glow up coach.</p>
                <button
                    onClick={() => router.push("/")}
                    className="mt-6 px-6 py-3 bg-primaryColor text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all"
                >
                    Back to Home
                </button>
            </div>
            ...
            <main className="max-w-4xl mx-auto p-4 md:p-6 space-y-8">
                ...
                <section className="flex flex-col md:flex-row gap-8 items-center md:items-start animate-fadeIn">
                    ...
                    <div className="flex-1 w-full space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <ScoreBadge label="Current Glow" score={glowResult.overallScore} icon={Sparkles} color="hover:border-primaryColor/50" />
                            <ScoreBadge label="Potential" score={glowResult.potentialScore} icon={Target} color="hover:border-blue-400/50" />
                        </div>
                        ...
                        <div className="bg-white/60 backdrop-blur-md border border-white/80 rounded-3xl p-6 shadow-sm">
                            ...
                            {Object.entries(glowResult.facialBreakdown).map(([key, score]) => (
                                <FeatureBar key={key} label={key} score={score} />
                            ))}
                        </div>
                    </div>
                </section>
                ...
                <section className="grid md:grid-cols-2 gap-6 animate-fadeInDelay">
                    <div className="bg-green-50/50 border border-green-100 rounded-3xl p-6">
                        ...
                        <ul className="space-y-3">
                            {glowResult.strengths.map((str, i) => (
                    ...
                    <div className="bg-amber-50/50 border border-amber-100 rounded-3xl p-6">
                        ...
                        <ul className="space-y-3">
                            {glowResult.improvementAreas.map((area, i) => (
                ...
                <section className="space-y-6 animate-fadeInDelay2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <TipCard title="hair style" tips={glowResult.maxxingTips.hairstyle} icon={Scissors} />
                        <TipCard title="grooming" tips={glowResult.maxxingTips.grooming} icon={Zap} />
                        <TipCard title="skincare" tips={glowResult.maxxingTips.skincare} icon={Waves} />
                        <TipCard title="lifestyle" tips={glowResult.maxxingTips.lifestyle} icon={Heart} />
                    </div>
                </section>
